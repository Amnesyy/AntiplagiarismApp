...
def generate_comparative_files(output_dir=Path("./comparative_files"), num_files=4, length=50000):
    """
    Generate large comparative files with random content.

    Args:
        output_dir (Path): Directory to save the generated files.
        num_files (int): Number of comparative files to generate.
        length (int): Approximate length of each file in characters.
    ...
    for i in range(1, num_files + 1):
        file_path = output_dir / f"comp_file_{i}.txt"
...
def generate_file_to_compare(output_file, comparative_files, target_length=50000, noise_ratio=0.3):
    """
    Generate a file to compare with a target length of ~50,000 characters.

    Args:
        output_file (Path): Path to save the file to compare.
        comparative_files (list[Path]): List of comparative files to use for copying.
        target_length (int): Desired total length of the generated file.
        noise_ratio (float): Proportion of noise in the generated file (0.0 to 1.0).

    Returns:
        list[dict]: Tracking data for copied segments.
    """
    tracking_data = []
    total_length = 0

    def generate_random_noise(length):
        """Generate random noise of the specified length."""
    ...
    with open(output_file, "w") as output:
        while total_length < target_length:
            if random.random() < noise_ratio:
                # Add noise
                noise_length = random.randint(500, 2000)  # Random noise segment size
                noise = generate_random_noise(noise_length)
                output.write(noise)
                total_length += noise_length
            else:
                # Copy content from a comparative file
                ...
                if len(comp_clean_content) > 0:
                    ...
                    # Optionally insert noise within the copied content
                    if random.random() < 0.5 and length > 10:
                        insertion_point = random.randint(1, length - 1)
                        noise = generate_random_noise(random.randint(10, 50))
                        copied_segment = (
                            copied_segment[:insertion_point] + noise + copied_segment[insertion_point:]
                        )
                        length += len(noise)
        ...
        for k in k_values:
            if algo == "winnowing":
                ...
                for modulo in modulo_values:
                    ...
                    if config not in tested_configurations:
                        tested_configurations.add(config)
                        run_app(executable, k, w=0, algo=algo, modulo=modulo)  # W is irrelevant
...
if __name__ == "__main__":
    ...
    k_values = [3, 5]  # K-gram lengths
    w_values = [4, 6]  # Window lengths
    algorithms = ["modulo", "winnowing"]  # Algorithms to test
    modulo_values = [7, 11]  # Modulo values for 'modulo' algorithm
